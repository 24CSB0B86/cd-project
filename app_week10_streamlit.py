"""
Run:
    streamlit run app_week10_streamlit.py
"""
...
.pipeline-step.active .pipeline-dot { background: #818cf8; }
...
═══════════════════════════════════════════════ */
.top-bar {
    background: linear-gradient(135deg, #1a1f2e 0%, #161b27 60%, #1a1025 100%);
...
.metric-card.lines::after    { background: linear-gradient(90deg, #60a5fa, #3b82f6); }
...
.metric-card.lines .val    { color: #60a5fa; }
...
.stSelectbox svg { color: #8b949e !important; }
...
/* Override for quick buttons specifically */
...
def _init():
    ...
    # Default to Web API if keyed, else Web Offline
    ...
    def _run_tool(label, fn):
        if st.button(label, use_container_width=True):
            if st.session_state.chat_session and st.session_state.chat_session.toolbox:
                ...
                st.toast("Please compile code first to start a session!", icon="⚠️")
...
<div class="top-bar">
  <div>
    ...
    <div class="top-bar-sub">Project 83 &mdash; Week 10 · Conversational Debugging Interface</div>
  </div>
...
# ─────────────────────────────────────────────────────────────────────────────
# Screen Rendering (Web or CLI representation)
# ─────────────────────────────────────────────────────────────────────────────
if is_cli:
    ...
    cmd_args = "--interactive --api" if use_api else "--interactive"
    cmd = f"python demo_week10_chat.py {cmd_args}"
    
    st.markdown(f'<div class="empty-state-hint" style="margin-bottom: 20px;">'
                f'You have selected the Command Line Interface mode.<br>'
                f'You can launch it directly from here or run it in your own terminal:<br><br>'
    ...
    if cols[1].button("🚀  Launch Terminal Window", use_container_width=True):
        ...
        st.toast("CLI window has been launched!", icon="✅")
        
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown('<p class="app-footer">Project 83 &nbsp;·&nbsp; AI Compiler Assistant &nbsp;·&nbsp; Week 10</p>', unsafe_allow_html=True)
    st.stop()

# ─────────────────────────────────────────────────────────────────────────────
# Code editor (Web Mode only)
# ─────────────────────────────────────────────────────────────────────────────
SAMPLES = {
    "— select a sample —": "",
    "✅  Factorial function": """\
...
with st.expander("📝  Code Editor", expanded=not st.session_state.code_loaded):
    ...
    code_input = st.text_area(
        "Source code:",
        value=st.session_state["_textarea_value"],
        height=220,
...
<div class="metric-row">
  <div class="metric-card errors">
    <div class="val">{s.error_count}</div>
  ...
  <div class="metric-card turns">
    <div class="val">{s.get_turn_count()}</div>
    <div class="lbl">Turns</div>
  </div>
  <div class="metric-card lines">
    <div class="val">{len(s.source_code.splitlines())}</div>
...
<div class="msg-row-assistant">
  ...
  <div>
    ...
    quick_qs = [
        ...
        "Explain the AST structure",
    ]
    for i, (qc, qq) in enumerate(zip(qcols, quick_qs)):
        if qc.button(qq, key=f"qq{i}", use_container_width=True):
            if not st.session_state.chat_session:
                st.toast("Please 'Compile & Start Session' first!", icon="⚠️")
            else:
                with st.spinner("Thinking..."):
                    resp = st.session_state.debugger.send_message(
                        st.session_state.chat_session, qq)
...
st.markdown("<hr>", unsafe_allow_html=True)
st.markdown(
    '<p class="app-footer">Project 83 &nbsp;·&nbsp; AI Compiler Assistant &nbsp;·&nbsp; Week 10</p>',
    unsafe_allow_html=True)
